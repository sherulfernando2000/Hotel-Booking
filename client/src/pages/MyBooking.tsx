        ...
        {bookings.map((booking)=>(
            <div key={booking._id} className='grid grid-cols-1  md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 py-6 first:border-t'>
                ...
                <div className='flex flex-col md:flex-row'>
                    ...
                    className='min-md:w-38 rounded shadow object-cover'/>
                ...
                <div className='flex flex-col items-start justify-center pt-3'>
                    <div className='flex item-center gap-2'>
                        <div className={`m-1 h-3 w-3 rounded-full ${booking.isPaid? "bg-green-500": "bg-red-500"}`}></div>
                        <p className={`text-sm ${booking.isPaid? "text-green-500": "text-red-500"}`}>
                            {booking.isPaid? "Paid":"Unpaid"}
                        </p>
                    </div>
                    {!booking.isPaid && (
                        <button className='px-4 py-1.5 mt-4 text-xs border border-gray-400 rounded-full hover:bg-gray-300 transition-all cursor-pointer'>PayNow</button>
                    )}
                </div>
            </div>
        ))}
